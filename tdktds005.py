...
tim = "\033[1;35m"
lam = "\033[1;36m"
thanh = f'{red}[{trang}TDK{red}] {trang}➜' 
thanh_ngang_dep = f'{luc}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'
...
def banner():
  ...
  ╚══██╔══╝██╔══██╗██║ ██╔╝
     ...
     ╚═╝   ╚═════╝ ╚═╝  ╚═╝
 {thanh_ngang_dep}
 {red}[{trang}TDK{red}] {luc}Tool Trao Đổi Sub Facebook - ZALO: https://zalo.me/g/ddxsyp497
 {red}[{trang}TDK{red}] {luc}Admin: {vang}DUONG PHUNG
 {thanh_ngang_dep}
''')
...
def Delay(value):
    while value > 0:
        curr = str(value)[0:4]
        print(f'''{trang}[{lam}TDK{trang}] [{vang}DELAY{trang}] [{red}{curr}s{trang}] {luc}● ❍ ❍ ❍      ''', end='\r'); sleep(0.04)
        print(f'''{trang}[{lam}TDK{trang}] [{vang}DELAY{trang}] [{red}{curr}s{trang}] {luc}❍ ● ❍ ❍      ''', end='\r'); sleep(0.04)
        print(f'''{trang}[{lam}TDK{trang}] [{vang}DELAY{trang}] [{red}{curr}s{trang}] {luc}❍ ❍ ● ❍      ''', end='\r'); sleep(0.04)
        print(f'''{trang}[{lam}TDK{trang}] [{vang}DELAY{trang}] [{red}{curr}s{trang}] {luc}❍ ❍ ❍ ●      ''', end='\r'); sleep(0.04)
        value -= 0.16
...
def Nhap_Cookie():
    ...
    while True:
        demck += 1
        print(f'{luc}● {trang}Nhập Cookie Facebook Thứ {vang}{demck}{trang}:', end=' ')
        ck = input(f'{vang}')
        if ck == '' and demck > 1:
            break
        fb = Facebook_Api(ck)
        ...
        if 'success' in info:
            ...
            uid = info['id']
            thanhngang(60)
            print(f'{thanh} {luc}ID{trang}: {vang}{uid} {red}| {luc}NAME{trang}: {vang}{name}')
            listck.append(ck)
            thanhngang(60)
        else:
            thanhngang(60)
            print(f'''{thanh} {luc}TRẠNG THÁI{trang}: {trang}[{red}DIE{trang}]\n{thanh} {luc}MSG{trang}: {luc}Đăng Nhập Thất Bại''')
            demck-=1
            thanhngang(60)
    return listck
...
def Nhap_Setting():
    apikey = ""
    print(f'{thanh} {luc}Delay Min{trang}: {vang}', end=''); min = int(input())
    print(f'{thanh} {luc}Delay Max{trang}: {vang}', end=''); max = int(input())
    print(f'{thanh} {luc}Chống Block Sau (NV){trang}: {vang}', end=''); nvblock = int(input())
    print(f'{thanh} {luc}Nghỉ Ngơi (Giây){trang}: {vang}', end=''); delaybl = int(input())
    print(f'{thanh} {luc}Đổi Nick Sau (NV){trang}: {vang}', end=''); doinick = int(input())
    print(f'{thanh} {luc}Lỗi (NV) Thì Đổi Cookie{trang}: {vang}', end=''); nhiemvuloi = int(input())
    config = {
        "apikey": apikey,
        "min": min,
        "max": max,
...
def Main():
    ...
    count = 0
    banner()
    print(f'{thanh} {luc}Proxy Format{trang}: {vang}username:password@host:port')
    thanhngang(60)
    print(f"{thanh} {luc}Nhập File Proxy (Enter = No Proxy){trang}: ", end=''); filename = input(f'{vang}')
    thanhngang(60)
    
    proxy_list = read_proxy_file(filename)
    proxy_index = 0
    current_proxy = None # Khởi tạo an toàn
    myip = None
    ...
    if proxy_list:
        ...
        try:
            ...
            if is_live == False:
                print(f'{luc}Proxy Không Hoạt Động!                            ', end = '\r');sleep(1); 
                myip = None
            else:
                print(f'{luc}Proxy Hoạt Động!                            ', end = '\r');sleep(1); 
    ...
    while True:
        if os.path.exists('acc_tds_log.txt'):
            with open('acc_tds_log.txt', 'r') as f:
                content = f.read()
                if '_' in content:
                    username, password = content.split('_')
                else:
                    # File lỗi thì xóa đi
            ...
            profile = tds.info()
            try:
                print(f'{thanh} {luc}Nhập {red}[{vang}1{red}] {luc}Chạy Tài Khoản: {vang}{profile[1]}')
                print(f'{thanh} {luc}Nhập {red}[{vang}2{red}] {luc}Nhập Tài Khoản Mới')
                thanhngang(60)
                print(f'{thanh} {luc}Lựa chọn{trang}: ', end=''); chon = input(f'{vang}')
                thanhngang(60)
                if chon == '2':
                    os.remove('acc_tds_log.txt')
                    continue
                ...
                print(f'''{thanh} {luc}Status{trang}: {trang}[{red}DIE{trang}]\n{thanh} {luc}Msg{trang}: {luc}Đăng Nhập Thất Bại''')
                os.remove('acc_tds_log.txt')
                continue
        
        if not os.path.exists('acc_tds_log.txt'):
            print(f'{thanh} {luc}User TDS{trang}: ', end=''); username = input(f'{vang}')
            print(f'{thanh} {luc}Pass TDS{trang}: ', end=''); password = input(f'{vang}')
            thanhngang(60)
        ...
        try:
            user = profile[1]
            xu = profile[2]
            print(f'''{thanh} {luc}Status{trang}: {trang}[{luc}LIVE{trang}]\n{thanh} {luc}Msg{trang}: {luc}Đăng Nhập Thành Công''')
            print(f'{thanh} {luc}Tài khoản{trang}: {vang}{user}')
    ...
    while True:
        if os.path.exists('Cookie_FB.txt'):
            print(f'{thanh} {luc}Nhập {red}[{vang}1{red}] {luc}Dùng Cookie Facebook Đã Lưu')
            print(f'{thanh} {luc}Nhập {red}[{vang}2{red}] {luc}Nhập Cookie Facebook Mới')
            thanhngang(60)
            print(f'{thanh} {luc}Lựa chọn{trang}: ', end=''); chon = input(f'{vang}')
            thanhngang(60)
            if chon == '1':
                print(f'{luc}Đang Load Dữ Liệu...')
                sleep(1)
    ...
    print(f'{thanh} {luc}Tài khoản{trang}: {vang}{user}')
    print(f'{thanh} {luc}Xu Hiện Tại{trang}: {vang}{str(format(int(xu),","))}')
    print(f'{thanh} {luc}Số Lượng FB{trang}: {vang}{len(listck)}')
    print(f'{thanh} {luc}IP Hiện Tại{trang}: {vang}{myip}')
    thanhngang(60)
    
    print(f'{red}[{vang}1{red}] {luc}Cảm Xúc Vip')
    print(f'{red}[{vang}2{red}] {luc}Cảm Xúc Cmt Vip')
    print(f'{red}[{vang}3{red}] {luc}Share Vip')
    print(f'{red}[{vang}4{red}] {luc}Follow Vip')
    print(f'{red}[{vang}5{red}] {luc}Like Page Vip')
    print(f'{red}[{vang}6{red}] {luc}Tham Gia Group')
    print(f'{lam}Chọn nhiều nhiệm vụ (Ví dụ: 1245)')
    thanhngang(60)
    print(f'{thanh} {luc}Nhập lựa chọn{trang}: ', end=''); nhap = input(f'{vang}')
    if '' in nhap:
        nhap = nhap.replace('','')
    listnv.append(nhap)
    thanhngang(60)
    ...
    if os.path.exists('settingch.json'):
        with open('settingch.json', 'r') as f:
            ...
            print(f'{thanh} {luc}Phát hiện cấu hình cũ:')
            print(f'{luc}   Delay: {vang}{min}-{max}s')
            print(f'{luc}   Chống Block: {vang}{nvblock}nv / {delaybl}s')
            print(f'{luc}   Đổi Nick: {vang}{doinick}nv')
            print(f'{luc}   Lỗi đổi Job: {vang}{nhiemvuloi}nv')
        print(f'{thanh} {luc}Dùng lại cấu hình cũ? {red}({vang}y/n{red}){trang}: ', end=''); chon = input(f'{vang}')
        if chon == 'y':
            pass
        else:
    ...
    print(f'{thanh} {luc}Ẩn ID Facebook? {red}({vang}y/n{red}){trang}: ', end=''); chonan = input(f'{vang}')
    thanhngang(60)
    
    while True:
        if len(listck) == 0:
            print(f'{red}Đã hết Cookie, vui lòng nhập lại!')
            listck = Nhap_Cookie()
            with open('Cookie_FB.txt', 'w') as f:
                json.dump(listck, f)
        ...
        for ck in listck:
            nhiemvu = listnv[0]
            loireaction, loicxcmt, loishare, loifollow, loipage, loigr = 0, 0, 0, 0, 0, 0
            
            # Xử lý Proxy xoay vòng
            proxy_list = read_proxy_file(filename)
            if proxy_list:
                current_proxy = proxy_list[proxy_index]
                proxy_index = (proxy_index + 1) % len(proxy_list)
                try:
                    ...
                    is_live = check['data'][0]['isLive']
                    if is_live == False:
                        print(f'{luc}Proxy Die! Reconnecting...                            ', end = '\r')
                        myip = None
                    else:
                        print(f'{luc}Proxy Live! Connecting...                            ', end = '\r')
                        myip = check['data'][0]['ip']
                except:
                    myip = "Proxy Err"
            else:
            ...
            if 'success' in info:
                ...
                uid_temp = ck.split('c_user=')[1].split(';')[0]
                print(f'{red}Cookie {uid_temp} Die!', end='\r');sleep(1); print('                                     ', end = '\r' )
                listck.remove(ck)
                continue
            ...
            if cauhinh == True:
                print(f'{luc}RUN ID: {vang}{uid2} {red}| {luc}Name: {vang}{name}')
            else:
                if apikey:
                    print(f'{luc}Đang Thêm ID: {vang}{uid} {red}| {luc}Name: {vang}{name}')
                    ...
                    if get_g_recaptcha_response[0] == True:
                        ...
                        if add_uid[0] == True:
                            cauhinh = tds.facebook_configuration(uid)
                            if cauhinh == True:
                                print(f'{luc}Cấu Hình OK: {vang}{uid2}')
                            else:
                                print(f'{luc}Cấu Hình Lỗi: {vang}{uid}')
                                listck.remove(ck)
                                continue
                        else:
                            print(f'{luc}Thêm UID Lỗi: {vang}{uid}')
                            listck.remove(ck)
                            continue
                    else:
                        print(f'{luc}Captcha Lỗi: {vang}{uid}')
                        listck.remove(ck)
                        continue
                else:
                    print(f'{luc}Chưa Cấu Hình ID: {vang}{uid}')
                    listck.remove(ck)
                    continue
            ...
            while True:
                ...
                if nhiemvu == '':
                    print(f'{red}Tài Khoản {vang}{name} {red}Block All!')
                    listck.remove(ck)
                    ptool = 1
                    break
                ...
                if '1' in nhiemvu:
                    ...
                    if listcx == False:
                         print(f'{luc}Đang tìm nhiệm vụ Cảm Xúc...                           ', end = '\r');sleep(1)
                    elif 'error' in listcx:
                        if listcx['error'] == 'Thao tác quá nhanh vui lòng chậm lại':
                            count = listcx['countdown']
                            print(f'{luc}Đợi nhiệm vụ {vang}Cảm Xúc, {luc}Time: {str(round(count, 3))}s              ', end = '\r')
                            Delay(count)
                        else:
                            print(listcx['error'], end = '\r'); sleep(1)
                    else:
                        list_nv = listcx['data']
                        if len(list_nv) == 0:
                            print(f'{luc}Hết nhiệm vụ Cảm Xúc...                           ', end = '\r');sleep(1)
                        else:
                            print(f'{luc}Tìm thấy {vang}{len(list_nv)} {luc}nhiệm vụ Cảm Xúc                      ', end = '\r')
                            for x in list_nv:
                                idpost = x['id']
                                id = idpost.split('_')[1] if '_' in idpost else idpost
                                id2_show = id[:3]+'#'*(len(id)-6)+id[-3:]
                                code = x['code']
                                type = x['type']
                                like = fb.reaction(id, type)
                                if like == False:
                                    print(f"{luc}FAIL {lam}{type}{trang}: {vang}{id}            ", end = '\r'); sleep(0)
                                    Delay(0)
                                    loireaction += 1
                                else:
                                    ...
                                    if 'success' in nhan:
                                        ...
                                        loireaction = 0
                                        dem+=1
                                        time_now = get_time_vn() # Giờ VN
                                        # FORM GỌN GÀNG MỚI
                                        print(f'{red}[{trang}{dem}{red}] {lam}| {vang}{time_now} {lam}| {trang}{type} {lam}| {vang}{id2_show} {lam}| {luc}{msg} {lam}| {vang}+{str(format(int(xu),","))} Xu {lam}| {trang}{myip}')
                                        if dem % doinick == 0:
                                            ptool = 1
                                            break
                                        if dem % nvblock == 0:
                                ...
                                if loireaction >= nhiemvuloi:
                                    ...
                                    checktt = fb2.info()
                                    if 'error' in checktt:
                                        print(f'{red}Cookie {vang}{name} {red}Checkpoint!                ')
                                        listck.remove(ck)
                                        ptool = 1
                                        break
                                    else:
                                        print(f'{red}Block {vang}Cảm Xúc {red}                           ', end = '\r');sleep(1)
                ...
                if '2' in nhiemvu:
                    ...
                    if listcxcmt == False:
                         print(f'{luc}Đang tìm nhiệm vụ CX CMT...                           ', end = '\r');sleep(1)
                    elif 'error' in listcxcmt:
                        if listcxcmt['error'] == 'Thao tác quá nhanh vui lòng chậm lại':
                            count = listcxcmt['countdown']
                            print(f'{luc}Đợi nhiệm vụ {vang}CX CMT, {luc}Time: {str(round(count, 3))}s              ', end = '\r')
                            Delay(count)
                        else:
                            print(listcxcmt['error'], end = '\r'); sleep(1)
                    else:
                        list_nv = listcxcmt['data']
                        if len(list_nv) == 0:
                            print(f'{luc}Hết nhiệm vụ CX CMT...                           ', end = '\r');sleep(1)
                        else:
                            print(f'{luc}Tìm thấy {vang}{len(list_nv)} {luc}nhiệm vụ CX CMT                     ', end = '\r')
                            for x in list_nv:
                                idpost = x['id']
                                id = idpost.split('_')[1] if '_' in idpost else idpost
                                id2_show = id[:3]+'#'*(len(id)-6)+id[-3:]
                                ...
                                if like == False:
                                    print(f"{luc}FAIL {lam}{type}CMT{trang}: {vang}{id}            ", end = '\r'); sleep(0)
                                    Delay(0)
                                    loicxcmt += 1
                                else:
                                    ...
                                    if 'success' in nhan:
                                        ...
                                        loicxcmt = 0
                                        dem+=1
                                        time_now = get_time_vn()
                                        print(f'{red}[{trang}{dem}{red}] {lam}| {vang}{time_now} {lam}| {trang}{type}CMT {lam}| {vang}{id2_show} {lam}| {luc}{msg} {lam}| {vang}+{str(format(int(xu),","))} Xu {lam}| {trang}{myip}')
                                        if dem % doinick == 0:
                                            ptool = 1
                                            break
                                        if dem % nvblock == 0:
                                ...
                                if loicxcmt >= nhiemvuloi:
                                    ...
                                    if 'error' in checktt:
                                        ...
                                        print(f'{red}Block {vang}CX CMT {red}                           ', end = '\r');sleep(1)
                ...
                if '3' in nhiemvu:
                    ...
                    if listshare == False:
                        print(f'{luc}Đang tìm nhiệm vụ Share...                           ', end = '\r');sleep(1)
                    elif 'error' in listshare:
                        if listshare['error'] == 'Thao tác quá nhanh vui lòng chậm lại':
                            count = listshare['countdown']
                            print(f'{luc}Đợi nhiệm vụ {vang}Share, {luc}Time: {str(round(count, 3))}s              ', end = '\r')
                            Delay(count)
                        else:
                            print(listshare['error'], end = '\r'); sleep(1)
                    else:
                        list_nv = listshare['data']
                        if len(list_nv) == 0:
                            print(f'{luc}Hết nhiệm vụ Share...                           ', end = '\r');sleep(1)
                        else:
                        ...
                        for x in list_nv:
                            ...
                            like = fb.share(id)
                            if like == False:
                                print(f"{luc}FAIL {lam}SHARE{trang}: {vang}{id}            ", end = '\r'); sleep(0)
                                Delay(0)
                                loishare += 1
                            else:
                                ...
                                if 'success' in nhan:
                                    ...
                                    dem+=1
                                    time_now = get_time_vn()
                                    print(f'{red}[{trang}{dem}{red}] {lam}| {vang}{time_now} {lam}| {vang}SHARE {lam}| {vang}{id2_show} {lam}| {luc}{msg} {lam}| {vang}+{str(format(int(xu),","))} Xu {lam}| {trang}{myip}')
                                    if dem % doinick == 0:
                                        ptool = 1
                                        break
                                    if dem % nvblock == 0:
                                ...
                                if loishare >= nhiemvuloi:
                                    ...
                                    if 'error' in checktt:
                                        ...
                                        print(f'{red}Block {vang}Share {red}                           ', end = '\r');sleep(1)
                ...
                if '4' in nhiemvu:
                    ...
                    if listfl == False:
                        print(f'{luc}Đang tìm nhiệm vụ Follow...                           ', end = '\r');sleep(1)
                    elif 'error' in listfl:
                        if listfl['error'] == 'Thao tác quá nhanh vui lòng chậm lại':
                            count = listfl['countdown']
                            print(f'{luc}Đợi nhiệm vụ {vang}Follow, {luc}Time: {str(round(count, 3))}s              ', end = '\r')
                            Delay(count)
                        else:
                            print(listfl['error'], end = '\r'); sleep(1)
                            tds.get_xu_vip('facebook_follow', 'facebook_api')
                    else:
                        list_nv = listfl['data']
                        if len(list_nv) == 0:
                            print(f'{luc}Hết nhiệm vụ Follow...                           ', end = '\r');sleep(1)
                        else:
                            ...
                            for x in list_nv:
                                ...
                                like = fb.follow(id)
                                if like == False:
                                    print(f"{luc}FAIL {lam}FOLLOW{trang}: {vang}{id}            ", end = '\r'); sleep(0)
                                    Delay(0)
                                    loifollow += 1
                                else:
                                    ...
                                    time_now = get_time_vn()
                                    # Hiển thị cache
                                    print(f'{red}[{trang}{dem}{red}] {lam}| {vang}{time_now} {lam}| {vang}FOLLOW {lam}| {vang}{id2_show} {lam}| {trang}Cache {cache}{luc}/{vang}5')
                                    if dem % doinick == 0:
                                        ptool = 1
                                        break
                                    if dem % nvblock == 0:
                                    ...
                                    if cache >= 5:
                                        ...
                                        if 'success' in nhan:
                                            ...
                                            time_now = get_time_vn()
                                            print(f'{red}[{trang}*{red}] {lam}| {vang}{time_now} {lam}| {luc}NHẬN XU {lam}| {trang}Jobs: {job_success} {lam}| {luc}{msg} {lam}| {vang}+{str(format(int(xu),","))} Xu {lam}| {trang}{myip}')
                                ...
                                if loifollow >= nhiemvuloi:
                                    ...
                                    if 'error' in checktt:
                                        ...
                                        print(f'{red}Block {vang}Follow {red}                           ', end = '\r');sleep(1)
                ...
                if '5' in nhiemvu:
                    ...
                    if listpage == False:
                         print(f'{luc}Đang tìm nhiệm vụ Page...                           ', end = '\r');sleep(1)
                    elif 'error' in listpage:
                        if listpage['error'] == 'Thao tác quá nhanh vui lòng chậm lại':
                            count = listpage['countdown']
                            print(f'{luc}Đợi nhiệm vụ {vang}Page, {luc}Time: {str(round(count, 3))}s              ', end = '\r')
                            Delay(count)
                        else:
                            print(listpage['error'], end = '\r'); sleep(1)
                    else:
                        list_nv = listpage['data']
                        if len(list_nv) == 0:
                            print(f'{luc}Hết nhiệm vụ Page...                           ', end = '\r');sleep(1)
                        else:
                            print(f'{luc}Tìm thấy {vang}{len(list_nv)} {luc}nhiệm vụ Page                     ', end = '\r')
                            for x in list_nv:
                                idpost = x['id']
                                id = idpost.split('_')[1] if '_' in idpost else idpost
                                id2_show = id[:3]+'#'*(len(id)-6)+id[-3:]
                                ...
                                if like == False:
                                    print(f"{luc}FAIL {lam}PAGE{trang}: {vang}{id}            ", end = '\r'); sleep(0)
                                    Delay(0)
                                    loipage+=1
                                else:
                                    ...
                                    loipage = 0
                                    time_now = get_time_vn()
                                    print(f'{red}[{trang}{dem}{red}] {lam}| {vang}{time_now} {lam}| {vang}PAGE {lam}| {vang}{id2_show} {lam}| {trang}Cache {cache}{luc}/{vang}5')
                                    if dem % doinick == 0:
                                        ptool = 1
                                        break
                                    if dem % nvblock == 0:
                                ...
                                if loipage >= nhiemvuloi:
                                    ...
                                    if 'error' in checktt:
                                        ...
                                        print(f'{red}Block {vang}Page {red}                           ', end = '\r');sleep(1)
                ...
                if '6' in nhiemvu:
                    ...
                    if listgr == False:
                        print(f'{luc}Đang tìm nhiệm vụ Group...                           ', end = '\r');sleep(1)
                    elif 'error' in listgr:
                        if listgr['error'] == 'Thao tác quá nhanh vui lòng chậm lại':
                            count = listgr['countdown']
                            print(f'{luc}Đợi nhiệm vụ {vang}Group, {luc}Time: {str(round(count, 3))}s              ', end = '\r')
                            Delay(count)
                        else:
                            print(listgr['error'], end = '\r'); sleep(1)
                    else:
                        list_nv = listgr
                        if len(list_nv) == 0:
                            print(f'{luc}Hết nhiệm vụ Group...                           ', end = '\r');sleep(1)
                        else:
                            print(f'{luc}Tìm thấy {vang}{len(list_nv)} {luc}nhiệm vụ Group                     ', end = '\r')
                        for x in list_nv:
                            idpost = x['id']
                            id2_show = idpost[:3]+'#'*(len(idpost)-6)+idpost[-3:]
                            like = fb.group(idpost)
                            if like == False:
                                print(f"{luc}FAIL {lam}GROUP{trang}: {vang}{idpost}            ", end = '\r'); sleep(0)
                                Delay(0)
                                loigr += 1
                            else:
                                ...
                                if 'success' in nhan:
                                    ...
                                    dem+=1
                                    time_now = get_time_vn()
                                    print(f'{red}[{trang}{dem}{red}] {lam}| {vang}{time_now} {lam}| {vang}GROUP {lam}| {vang}{id2_show} {lam}| {luc}{msg} {lam}| {vang}+{str(format(int(xu),","))} Xu {lam}| {trang}{myip}')
                                    if dem % doinick == 0:
                                        ptool = 1
                                        break
                                    if dem % nvblock == 0:
                                ...
                                if loigr >= nhiemvuloi:
                                    ...
                                    if 'error' in checktt:
                                        ...
                                        print(f'{red}Block {vang}Group {red}                           ', end = '\r');sleep(1)
